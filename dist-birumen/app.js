(function () {
  ...
  function animateMatchCount() {
    ...
    requestAnimationFrame(function () {
      ...
      function count(now) {
        ...
        jobMatchCount.textContent = String(Math.round(targetJobCount * eased));
        if (progress < 1) {
  ...
  function stepIsValid() {
    ...
    const requiredGroups = Array.from(active.querySelectorAll("[data-required-group]"));
    if (requiredGroups.length) return requiredGroups.every((group) => group.querySelector("input:checked"));
  ...
  function renderStep() {
    ...
    if (currentStep === 1 && !matchHasAnimated) animateMatchCount();
    previousStep = currentStep;
  }

  form.addEventListener("change", function (event) {
    renderStep();
    if (steps[currentStep].hasAttribute("data-manual-step") || !autoAdvanceSteps.has(currentStep) || !event.target.matches('input[type="radio"]') || !event.target.checked) return;
